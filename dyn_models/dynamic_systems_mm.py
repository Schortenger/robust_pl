...
class Reorientation3:
    ...
    def forward_simulate(self, state_param, action):
        ...
        # tau_viscousity = param[:,2].view(-1,1)*dtheta
        # ddtheta = action[:,0].view(-1,1) - tau_gravity - tau_viscousity

        theta = theta + dtheta*self.dt

        # bottom_flag = (theta.abs()<5/180*torch.pi).float() #* (dtheta*theta<=0).float()
        # dtheta = (dtheta + ddtheta*self.dt)*(1 - bottom_flag) + (dtheta + ddtheta*self.dt + action[:,1].view(-1,1)) * bottom_flag
        dtheta = dtheta + ddtheta*self.dt

        # dtheta = torch.clip(dtheta, 0, self.state_max[1])
    ...
    def reward_state_action(self,state_param,action):
        ...
        # state[:, 0] = -1*state[:,0] * (state[:, 0]<=-torch.pi+5/180*torch.pi).float() + state[:,0] * (state[:, 0]>-torch.pi+5/180*torch.pi).float()
        # d_theta= ((state[:,0].abs()-torch.pi).abs()/0.01)**2
        r_goal = -1*((state[:,0].abs()-torch.pi).abs())/0.01 # -self.state_max[0]*10
        # r_goal = (state[:, 0].abs() -self.state_max[0]*20)
        # r_action  = (action[:,0].abs() - self.action_max[0]*20) + (action[:,1].abs() - self.action_max[1]*20)
        r_action = -1* torch.linalg.norm(action,dim=-1)

        # # d_action = torch.linalg.norm(action,dim=-1)
        # d_action = ((action[:,0]).abs()/self.action_max[0])**2 
        # # r_goal = -d_theta
        # r_action = -d_action
    ...
    #     # d_theta= ((state[:,0].abs()-torch.pi).abs()/0.01)**2
    #     theta = state[:,0]
    #     r_goal = -1*((state[:,0].abs()-torch.pi).abs())
    #     # r_goal = (state[:, 0].abs() -self.state_max[0]*20)
    #     # r_action  = (action[:,0].abs() - self.action_max[0]*20) + (action[:,1].abs() - self.action_max[1]*20)
    #     r_action = -1* torch.linalg.norm(action,dim=-1)
    ...
    #     r_plus = 0.5*((state[:, 0] -torch.pi).abs()<0.1) # + 0*(1- ((state[:, 0] -torch.pi).abs()<0.5))
        
    #     mask_flag = (state[:, 0] -torch.pi).abs()<0.1
    #     r_vel = -1*torch.linalg.norm(state[:, 1],dim=-1)*mask_flag - 5*(1-mask_flag.to(torch.int))

    #     return r_goal + r_action*0.01 #+ r_vel* 5 

class Reorientation_RL:
    '''
        Dynamics of a single pendulum
        state: (theta, dtheta) # (joinr_angle,joint_vel), joint_angle in (-pi, pi), it is 0 at the stable equilibrium
        action: (u) # joint torquw
...
class Reorientation_MotorAdapt:
    '''
        Dynamics of a single pendulum
        state: (theta, dtheta) # (joinr_angle,joint_vel), joint_angle in (-pi, pi), it is 0 at the stable equilibrium
        action: (u) # joint torquw
    '''
    def __init__(self, mlp_encoder, state_min, state_max, action_max, action_min, dt=0.01, w_scale=1.0, w_goal=0.9,w_action=0.1,device='cpu'):
